...
* 🛡️ **双轨智能容错**：支持网络 HTTP(S) 请求动态拉取，当直接双击本地 HTML 运行（受浏览器 CORS 跨域政策限制）或处于离线状态时，会自动智能降级并使用内置备份数据渲染，确保 100% 随时可用。
...
npx serve .
```
启动后通过浏览器访问对应地址即可。此时网页会自动动态加载 `data.json` 中的最新网址数据。

### 2. 本地双击运行
在无网络、无服务器环境下，直接在文件夹中**双击 `index.html` 即可运行**。网页会检测到 `file://` 协议限制，并立即启动智能降级，利用内置的初始备份数据为您极速渲染界面。
